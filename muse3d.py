...
DEFAULT_ROOM_ID = "SALA_VR"
MAX_ADDED_PAIR_COUNT = 24
NAVIGATION_GLB_FILES = ("incahuasi.glb", "ushnu.glb")
...
def prompt_choice(label: str, options: list[Path]) -> Path | None:
    ...
    for index, option in enumerate(options, start=1):
        try:
            ...
            try:
                display = Path("museApp") / option.relative_to(APP_ROOT)
            except ValueError:
                display = option
...
def discover_immersive_glbs() -> list[Path]:
    return [
        ...
        for filename in NAVIGATION_GLB_FILES
...
def is_windows_blender(blender_bin: str) -> bool:
    lowered = blender_bin.lower()
    return lowered.endswith(".exe") or "blender.exe" in lowered


def path_for_blender(path: Path, blender_bin: str) -> str:
    if not is_windows_blender(blender_bin) or os.name == "nt":
        return str(path)
...
def send_mcp_request(
    ...
    *,
    ...
    raise RuntimeError("Blender MCP no devolvio una respuesta JSON.")


def execute_mcp_code(
    code: str,
    *,
    host: str = DEFAULT_MCP_HOST,
    port: int = DEFAULT_MCP_PORT,
...
def mcp_path(path: Path) -> str:
    return path_for_blender(path, "blender.exe")


def run_setup_via_mcp(
    *,
    description: str,
    host: str,
    model_path: Path,
...
setup = load_module("muse3d_setup_mcp", {json.dumps(mcp_path(SETUP_SCRIPT))})
old_argv = sys.argv[:]
sys.argv = [
    "blender",
    "--",
    {json.dumps(mcp_path(model_path))},
    "--points", {json.dumps(str(points))},
    "--route-id", {json.dumps(route_id)},
    "--route-model", {json.dumps(route_model)},
    "--description", {json.dumps(description)},
    "--save-blend", {json.dumps(mcp_path(workspace_path))},
]
try:
    setup.setup_tour()
finally:
    sys.argv = old_argv
"""
    execute_mcp_code(code, host=host, port=port, timeout_seconds=120)


def preview_tour_via_mcp(*, host: str, port: int) -> None:
    code = f"""
import argparse
import importlib.util
...
preview = load_module("muse3d_preview_mcp", {json.dumps(mcp_path(PROJECT_ROOT / "scripts" / "preview_immersive_tour.py"))})
preview.animate_preview_camera(argparse.Namespace(fps=24, start_frame=1, play=True))
"""
    execute_mcp_code(code, host=host, port=port, timeout_seconds=60)
    print("[Muse3D] Preview actualizado en Blender.")


def add_tour_pairs_via_mcp(*, count: int, host: str, port: int) -> None:
    code = f"""
import importlib.util
...
setup = load_module("muse3d_setup_mcp_add", {json.dumps(mcp_path(SETUP_SCRIPT))})
created = setup.add_tour_pairs({int(count)})
print(", ".join(created))
"""
    result = execute_mcp_code(code, host=host, port=port, timeout_seconds=60)
    print(f"[Muse3D] Pares Tour/Target creados: {result.strip() or count}")


def refresh_view_via_mcp(*, host: str, port: int) -> None:
    code = f"""
import importlib.util
...
view = load_module("muse3d_view_mcp", {json.dumps(mcp_path(PROJECT_ROOT / "scripts" / "blender_view_setup.py"))})
view.apply_blender_preview_environment(rendered=True)
"""
    execute_mcp_code(code, host=host, port=port, timeout_seconds=60)
    print("[Muse3D] Vista actualizada con materiales/cielo.")


def export_route_via_mcp(*, host: str, port: int, route_path: Path) -> None:
    code = f"""
import importlib.util
from pathlib import Path
...
exporter = load_module("muse3d_export_mcp", {json.dumps(mcp_path(PROJECT_ROOT / "scripts" / "export_immersive_tour.py"))})
exporter.export_tour(Path({json.dumps(mcp_path(route_path))}))
"""
    execute_mcp_code(code, host=host, port=port, timeout_seconds=60)
...
def select_or_enter_model() -> Path:
    ...
    selected = prompt_choice("Selecciona el GLB navegable:", glbs)
    if selected:
    ...
    *,
    ...
    try:
        status = send_blender_command(
            ...
            {
                "action": "export",
                "output": path_for_blender(route_path, blender_bin),
            },
            timeout_seconds=30,
        )
...
def export_and_sync_experience_via_mcp(
    *,
    cta_label: str,
    description: str,
    experience_id: str,
    ...
    try:
        export_route_via_mcp(host=host, port=port, route_path=route_path)
        print(f"[Muse3D] Ruta exportada: {route_path}")
    except RuntimeError as error:
        print(f"[Muse3D] No se pudo exportar via MCP: {error}")
        return False

    sync_exported_experience(
        cta_label=cta_label,
        description=description,
...
def run_mcp_edit_loop(
    *,
    ...
    print()
    print("[Muse3D] Usando Blender abierto via MCP.")
    print("[Muse3D] Se cargara el GLB en la escena actual y se crearan Tour_XX/Target_XX.")
    run_setup_via_mcp(
        description=description,
        host=host,
        model_path=model_path,
    ...
    print("[Muse3D] Modelo cargado. Ajusta Tour_XX y Target_XX en Blender.")

    while True:
        print()
        print(f"Muse3D - Edicion de tour (MCP {host}:{port})")
        ...
        if option == "1":
            try:
                preview_tour_via_mcp(host=host, port=port)
            except RuntimeError as error:
                print(f"[Muse3D] No se pudo previsualizar via MCP: {error}")
        elif option == "2":
            requested_count = prompt_int("Cantidad de pares Tour/Target a agregar", 1)
            count = min(requested_count, MAX_ADDED_PAIR_COUNT)
            if count != requested_count:
                print(f"[Muse3D] Por seguridad se agregaran {MAX_ADDED_PAIR_COUNT} pares como maximo.")
            try:
                add_tour_pairs_via_mcp(count=count, host=host, port=port)
            except RuntimeError as error:
                print(f"[Muse3D] No se pudieron agregar pares via MCP: {error}")
        elif option == "3":
            exported = export_and_sync_experience_via_mcp(
                cta_label=cta_label,
                description=description,
                experience_id=experience_id,
                export_name=export_name,
            ...
            try:
                refresh_view_via_mcp(host=host, port=port)
            except RuntimeError as error:
                print(f"[Muse3D] No se pudo actualizar la vista via MCP: {error}")
        elif option == "5":
            print("[Muse3D] Flujo terminado. Blender queda bajo tu control.")
            return
        else:
    ...
    *,
    ...
    blender_process = launch_command(
        [
            blender_bin,
            "--python",
            path_for_blender(SETUP_SCRIPT, blender_bin),
            "--",
            path_for_blender(model_path, blender_bin),
            "--points",
            str(points),
            "--route-id",
            route_id,
            ...
            description,
            "--save-blend",
            path_for_blender(workspace_path, blender_bin),
            "--command-file",
            path_for_blender(command_file, blender_bin),
            "--status-file",
            path_for_blender(status_file, blender_bin),
        ]
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--mcp",
        action="store_true",
        help="Usa el Blender ya abierto por MCP en lugar de lanzar blender.exe.",
    )
